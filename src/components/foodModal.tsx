...
const FoodModal: React.FC<IFoodModalProps> = ({ visible, onClose }) => {
  ...
  const NotFoundFood = () => {
    return (
      <View
        className={`bg-color1 flex justify-center items-center rounded-3xl  shadow-2xl shadow-gray-900 `}
      >
        <View className="flex flex-row items-center gap-x-10">
          <ButtonIcon buttonIconName="close-circle" onPress={handleClose} />
          <Text className="font-bold text-2xl text-color3 mt-2 text-center">
  ...
  const SearchFood = () => {
    return (
      ...
      >
        ...
        <View className="flex flex-col justify-between mb-4 w-full px-4 ">
          <TextButton
            text=" Nao encontrou o alimento? clique aqui para cadastrar ele"
